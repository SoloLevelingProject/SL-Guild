...
public class CustomHeadSupport {
    ...
    public static void setupCustomHeadJsonFiles() {
        ...
        File customHeadsFolder = new File(BangHoi.plugin.getDataFolder() + "/" + customHeadsFolderName);
        ...
        for (CustomHeadCategory customHeadCategory : CustomHeadCategory.values()) {
            ...
            if (new File(BangHoi.plugin.getDataFolder() + "/" + customHeadsFolderName + "/custom-head-" + customHeadCategoryString + ".json").exists())
                continue;

            try (FileWriter file = new FileWriter(BangHoi.plugin.getDataFolder() + "/" + customHeadsFolderName + "/custom-head-" + customHeadCategoryString + ".json")) {
                if (!Settings.CUSTOM_HEADS_API_V2_ENABLED)
