...
public class Create extends SubjectManager {
    ...
    public boolean execute() {
        ...
        if (clanName.length() < Settings.CLAN_SETTING_NAME_MINIMUM_LENGTH) {
            MessageUtil.sendMessage(player, Messages.ILLEGAL_MINIMUM_CLAN_LENGTH.replace("%minimumClanNameLength%",
                    String.valueOf(Settings.CLAN_SETTING_NAME_MINIMUM_LENGTH)));
            return false;
        }

        if (clanName.length() > Settings.CLAN_SETTING_NAME_MAXIMUM_LENGTH) {
            MessageUtil.sendMessage(player, Messages.ILLEGAL_MAXIMUM_CLAN_LENGTH.replace("%maximumClanNameLength%",
                    String.valueOf(Settings.CLAN_SETTING_NAME_MAXIMUM_LENGTH)));
            return false;
        }
        ...
        // Because of windows limited, these characters cannot be contained in the clan
        // name
        ...
        for (String prohibitedCharacter : prohibitedCharacters) {
            if (clanName.contains(prohibitedCharacter)) {
                MessageUtil.sendMessage(player,
                        Messages.PROHIBITED_CHARACTER.replace("%character%", prohibitedCharacter));
                return false;
            }
        }

        if (Settings.CLAN_SETTING_CREATE_ENABLED)
            if (!UpgradeManager.checkPlayerCurrency(player,
                    CurrencyType.valueOf(Settings.CLAN_SETTING_CREATE_TYPE.toUpperCase()),
                    Settings.CLAN_SETTING_CREATE_CURRENCY, true))
                return false;
        ...
        String selectedMessage = Settings.CLAN_SETTINGS_CREATION_BROADCAST.get(randomBroadcastLine)
                .replace("%clanName%", clanName).replace("%player%", playerName);
