...
public class ClanAdminCommand implements CommandExecutor, TabExecutor {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("backup")) {
                ...
                BangHoi.support.getFoliaLib().getScheduler().runAsync(task -> {
                    ...
                    sender.sendMessage(
                            "Backup successful! Backup file will be in the 'backup' folder inside the plugin directory.");
                    sender.sendMessage("Database type: " + BangHoi.databaseType.toString().toUpperCase());
                });
                return false;
            }
            ...
            if (args[0].equalsIgnoreCase("debug")) {
                ...
                sender.sendMessage("Debug mode " + (Settings.DEBUG_ENABLED ? "§aenabled" : "§cdisabled") + "§r.");
                return false;
            }
        }
        ...
        if (args.length >= 2) {
                ...
                if (!PluginDataManager.getClanDatabase().containsKey(clanName)) {
                    ...
                    if (clanDataType == ClanDataType.createddate) {
                        try {
                            ...
                            if (args[3].equalsIgnoreCase("set")) {
                                ...
                                sender.sendMessage("Set " + clanName + " " + value + " " + clanDataType);
                                sender.sendMessage(
                                        clanName + "''s new " + clanDataType + ": " + clanData.getCreatedDate());
                            }
                        } catch (Exception exception) {
                            sender.sendMessage("Value is not available.");
                            return false;
                    ...
                    if (clanDataType == ClanDataType.subjectpermission) {
                        if (args[3].equalsIgnoreCase("reset")) {
                            ...
                            for (Subject subject : Subject.values())
                                newPermissionDefault.put(subject,
                                        Settings.CLAN_SETTING_PERMISSION_DEFAULT.get(subject));
                    ...
                    if (clanDataType == ClanDataType.discordchannelid) {
                        ...
                        try {
                            ...
                            if (args[3].equalsIgnoreCase("set")) {
                                ...
                                sender.sendMessage("Set " + clanName + " " + value + " " + clanDataType);
                                sender.sendMessage(
                                        clanName + "''s new " + clanDataType + ": " + clanData.getDiscordChannelID());
                            }
                        } catch (Exception exception) {
                            sender.sendMessage("Value is not available.");
                            return false;
                    ...
                    if (clanDataType == ClanDataType.members) {
                        if (args[3].equalsIgnoreCase("add")) {
                            ...
                            if (ClanManager.isPlayerInClan(playerName)) {
                                ...
                                sender.sendMessage("Note: You can remove " + playerName + " from their current clan ("
                                        + PluginDataManager.getPlayerDatabase(playerName).getClan()
                                        + ") using the setPlayerData command in clanAdmin.");
                                return false;
                            }
                    ...
                    if (clanDataType == ClanDataType.allies) {
                        ...
                        if (args[3].equalsIgnoreCase("remove")) {
                            ...
                            sender.sendMessage(
                                    "Removed clan " + allyName + " from the ally list of clan " + clanName + ".");
                            return false;
                        }
                    }
            ...
            if (args[0].equalsIgnoreCase("transferplugindatabasetype")) {
                try {
                    ...
                    if (databaseType != BangHoi.databaseType) {
            ...
            if (args[0].equalsIgnoreCase("backup")) {
                ...
                BangHoi.support.getFoliaLib().getScheduler().runAsync(task -> {
                    ...
                    sender.sendMessage(
                            "Backup successful! The backup file will be located in the 'backup' folder inside the plugin directory.");
                    sender.sendMessage("Database type: " + BangHoi.databaseType.toString().toUpperCase());
                ...
                try {
                        ...
                        if (args[3].equalsIgnoreCase("set")) {
                            ...
                            if (ClanManager.isPlayerInClan(playerName)) {
                                if (PluginDataManager.getPlayerDatabase(playerName).getRank() == Rank.LEADER) {
                                    sender.sendMessage(playerName + " is a owner of "
                                            + PluginDataManager.getPlayerDatabase(playerName).getClan()
                                            + ". Please reset instead of setting!");
                                    return false;
                                }
                            }
                            ClanManager.addPlayerToAClan(playerName, clanName, true);
                            ...
                            sender.sendMessage(
                                    "Database of " + clanName + " also added " + playerName + " to members list.");
                            return false;
                        }
                        if (args[3].equalsIgnoreCase("reset")) {
                            if (!ClanManager.isPlayerInClan(playerName)) {
                    ...
                    if (playerDataType.equals(PlayerDataType.joindate)) {
                        if (args[3].equalsIgnoreCase("set")) {
                            ...
                            sender.sendMessage("Set ngày tham gia of " + playerName + " to " + newJoinDate + " ("
                                    + com.banghoi.util.StringUtil.dateTimeToDateFormat(newJoinDate));
                            return false;
                        }
                    }
                    ...
                    if (playerDataType.equals(PlayerDataType.lastactivated)) {
                        if (args[3].equalsIgnoreCase("set")) {
                            ...
                            sender.sendMessage("Set last activated time for " + playerName + " to " + newLastActivated
                                    + " (" + com.banghoi.util.StringUtil.dateTimeToDateFormat(newLastActivated));
                            return false;
                        }
                    }
                } catch (Exception exception) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("setclandata") || args[0].equalsIgnoreCase("delete")) {
                if (!PluginDataManager.getClanDatabase().isEmpty()) {
            ...
            if (args[0].equalsIgnoreCase("transferPluginDatabaseType")) {
                if (!transferDataCommandNotifying.contains(sender)) {
                    sender.sendMessage("-----------------------");
                    sender.sendMessage("Note: The current clan database type is: "
                            + BangHoi.databaseType.toString().toUpperCase());
                    ...
                    sender.sendMessage(
                            "For data safety during the database type transfer, please perform this while the server has no players online.");
                    sender.sendMessage("-----------------------");
                    transferDataCommandNotifying.add(sender);
                    BangHoi.support.getFoliaLib().getScheduler().runLater(() -> {
            ...
            if (args[0].equalsIgnoreCase("setclandata")) {
                if (PluginDataManager.getClanDatabase().containsKey(args[1])) {
                    ...
                    if (type.equalsIgnoreCase("score") || type.equalsIgnoreCase("warning")) {
                        commands.add("give");
                        commands.add("remove");
                    }
                }
    ...
    private enum ClanDataType {
        score, warning, createddate, customname, message, icon, spawnpoint, subjectpermission, discordchannelid,
        discordjoinlink, members, allies
    }
