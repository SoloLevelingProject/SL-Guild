...
import com.banghoi.file.inventory.MemberListInventoryFile;
        ...
        BangHoi.support.getFoliaLib().getScheduler().runAsync(task -> {
            ...
            if (sortItemType == SortItemType.SCORECOLLECTED) {
                ...
                for (String member : PluginDataManager.getClanDatabase(clanName).getMembers()) {
                    IPlayerData memberData = PluginDataManager.getPlayerDatabase(member);
                    if (memberData == null) continue;
                    playersScore.put(member, memberData.getScoreCollected());
                }
            ...
            if (sortItemType == SortItemType.JOINDATE) {
                ...
                for (String member : PluginDataManager.getClanDatabase(clanName).getMembers()) {
                    IPlayerData memberData = PluginDataManager.getPlayerDatabase(member);
                    if (memberData == null) continue;
                    playersJoinDate.put(member, memberData.getJoinDate());
                }
            ...
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (players.get(index) != null) {
                    ...
                    ItemStack itemStack = BangHoi.nms.addCustomData(ItemUtil.getPlayerItemStack(playerItem, playerName), "player=" + playerName);
