...
import com.banghoi.listener.SignChangeListener;
...
public abstract class PaginatedInventory extends BangHoiInventoryBase {
    ...
    public void addPaginatedMenuItems(FileConfiguration fileConfiguration, boolean backButton) {
            ...
            if (getSlots() > 18)
                for (int i = 0; i < 10; i++) {
                    if (inventory.getItem(i) == null) {
                        inventory.setItem(i, BangHoi.nms.addCustomData(borderItem, "border"));
                    }
                }
            if (getSlots() > 27) {
        ...
        ItemStack searchItem = BangHoi.nms.addCustomData(ItemUtil.getItem(
    ...
    @NotNull ItemStack getPageItemStack(ItemStack itemStack) {
        ...
        itemLore.replaceAll(string -> BangHoi.nms.addColor(string.replace("%page%", String.valueOf(getPage()))
